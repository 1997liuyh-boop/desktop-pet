...
class EnhancedStats {
  ...
  getMood() {
    ...
    if (f >= MOOD_THRESHOLDS.POOR_MIN_FEELING || h >= MOOD_THRESHOLDS.POOR_MIN_HEALTH) return ModeType.POOR_CONDITION;
    return ModeType.ILL;
  }

  getMoodText() {
    const map = { [ModeType.HAPPY]: '开心', [ModeType.NORMAL]: '正常', [ModeType.POOR_CONDITION]: '不适', [ModeType.ILL]: '生病' };
    return map[this.getMood()] || '正常';
  }

  update(dt) {
    const now = Date.now();
    const elapsed = Math.min((now - this._lastUpdate) / 1000, 1);
  ...
  loadFromObject(obj) {
    if (!obj) return;
    ['hunger', 'happiness', 'energy', 'health', 'strength', 'feeling', 'likability', 'level', 'exp', 'money'].forEach(k => {
      if (obj[k] !== undefined) this[k] = obj[k];
    });
